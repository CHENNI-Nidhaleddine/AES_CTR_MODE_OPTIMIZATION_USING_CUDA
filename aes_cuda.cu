...
__device__  const BYTE sbox[16][16] = {
 ...
 static const BYTE sbox_cpu[16][16] = {
    {0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76},
    {0xCA, 0x82, 0xC9, 0x7D, 0xFA, 0x59, 0x47, 0xF0, 0xAD, 0xD4, 0xA2, 0xAF, 0x9C, 0xA4, 0x72, 0xC0},
    {0xB7, 0xFD, 0x93, 0x26, 0x36, 0x3F, 0xF7, 0xCC, 0x34, 0xA5, 0xE5, 0xF1, 0x71, 0xD8, 0x31, 0x15},
    {0x04, 0xC7, 0x23, 0xC3, 0x18, 0x96, 0x05, 0x9A, 0x07, 0x12, 0x80, 0xE2, 0xEB, 0x27, 0xB2, 0x75},
...
static const BYTE invsbox_cpu[16][16] = {
...
__device__ static const BYTE invsbox[16][16] = {
    {0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38, 0xBF, 0x40, 0xA3, 0x9E, 0x81, 0xF3, 0xD7, 0xFB},
    {0x7C, 0xE3, 0x39, 0x82, 0x9B, 0x2F, 0xFF, 0x87, 0x34, 0x8E, 0x43, 0x44, 0xC4, 0xDE, 0xE9, 0xCB},
    {0x54, 0x7B, 0x94, 0x32, 0xA6, 0xC2, 0x23, 0x3D, 0xEE, 0x4C, 0x95, 0x0B, 0x42, 0xFA, 0xC3, 0x4E},
    {0x08, 0x2E, 0xA1, 0x66, 0x28, 0xD9, 0x24, 0xB2, 0x76, 0x5B, 0xA2, 0x49, 0x6D, 0x8B, 0xD1, 0x25},
...
__device__ static const BYTE gf_mul[256][6] = {
 ...
 WORD SubWord(WORD word)
{
    unsigned int result;
    // sbox[][]
    result = (int)sbox_cpu[(word >> 4) & 0x0000000F][word & 0x0000000F];
    result += (int)sbox_cpu[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F] << 8;
    result += (int)sbox_cpu[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F] << 16;
    result += (int)sbox_cpu[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F] << 24;
    return (result);
}
...
void keyExpansion(const BYTE key[], WORD w[], int keysize)
{
    int Nb = 4, Nr, Nk, idx;
    WORD temp, Rcon[] = {
...
__device__ void AddRoundKey(BYTE state[][4], const WORD w[])
{
    BYTE subkey[4];
...
__device__ void SubBytes(BYTE state[][4])
{
...
__device__ void InvSubBytes(BYTE state[][4])
{
...
__device__ void ShiftRows(BYTE state[][4])
{
    int t;
...
__device__ void InvShiftRows(BYTE state[][4])
{
    int t;
...
__device__ void MixColumns(BYTE state[][4])
{
    BYTE col[4];
...
__device__ void InvMixColumns(BYTE state[][4])
{
    BYTE col[4];
...
__device__ void encrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
    BYTE state[4][4];
    for (int i = 0; i < 4; i++)
    {
...
__device__ void decrypt(const BYTE in[], BYTE out[], const WORD key[], int keysize)
{
    BYTE state[4][4];
...
__device__ void xor_buffer(const BYTE in[], BYTE out[], size_t len)
{
    size_t idx;
...
void increment_ctr(BYTE ctr[], int counter_size)
{
    int idx;
...
__global__ void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE ctrs[])
{
    ...
    int numberOfGivenThreads = gridDim.x * blockDim.x;
    ...
    if (th_idx == gridDim.x * blockDim.x - 1) iterations = totalBlocksLastThread; 
    else iterations = totalBlocksParThread;
...
void aes_decrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE ctr[])
{
    // CTR encryption is its own inverse function.
    //aes_encrypt_ctr(in, in_len, out, key, keysize, ctr);
}
